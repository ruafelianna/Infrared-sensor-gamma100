...
let exn (exn:System.Exception) = match exn with  
                                 | :?  System.AggregateException as exn -> exn.InnerException.Message
                                 | _ as exn -> exn.Message
...
let test'crc16'null () = [|for i in [0uy..255uy]-> i|] |> add'crc16 |> crc16
...
let floatBcd6 (value:double) =     
    let rec c'v v =         
        match v*10. with 
        | 0. -> 0.1
        | next when (abs next)>999999. -> v
        | next -> c'v next
    let rec base'10 v d = 
        if v<pow 10. d then d else base'10 v (d+1)

    let comaPos = 6 - (base'10 value 0)
    
    let b8 = (if value<0. then 1uy else 0uy ) <<< 4
    let b7 = byte comaPos

    let v = value |> c'v |> round |> int
...
let casync f =  let dlgt = new System.Func<'a> ( f )
                Async.FromBeginEnd(dlgt.BeginInvoke, dlgt.EndInvoke)

module Chart =
    open System.Windows.Forms.DataVisualization.Charting 

    let getVisibleYOrders (chart:Chart) = 
        let ar = chart.ChartAreas.[0]
        let yAx, xAx = ar.AxisY, ar.AxisX
        let pts = [for series in chart.Series do 
                    for pt in series.Points do 
                        if pt.XValue>=xAx.Minimum && pt.XValue<=xAx.Maximum then 
                            yield pt]
        if pts |> List.isEmpty then None else
            Some(   pts |> List.map( fun point -> point.YValues |> Array.min ) |> List.min,
                    pts |> List.map( fun point -> point.YValues |> Array.max ) |> List.max)

    let normalizeVisibleY chart = 
        match getVisibleYOrders chart with
        | Some(min,max)  ->
            let d = max - min
            let d = if d=0. then 0.1 else d
            let ar = chart.ChartAreas.[0]
            let yAx, xAx = ar.AxisY, ar.AxisX            
            yAx.Minimum <- min - 0.05*d
            yAx.Maximum <- max + 0.05*d
        | _ ->()

    let getOrders (chart:Chart) = 
        let ar = chart.ChartAreas.[0]
        let yAx, xAx = ar.AxisY, ar.AxisX
        let pts = [for series in chart.Series do for pt in series.Points -> pt]
        if pts |> List.isEmpty then None else
            Some(   pts |> List.map( fun point -> point.XValue ) |> List.min,
                    pts |> List.map( fun point -> point.XValue ) |> List.max, 
                    pts |> List.map( fun point -> point.YValues |> Array.min ) |> List.min,
                    pts |> List.map( fun point -> point.YValues |> Array.max ) |> List.max)

    let normalize chart = 
        let sax minv maxv (ax:Axis)  = 
            let d = maxv - minv
            let d = if d=0. then 0.1 else d
            ax.Minimum <- minv - 0.1*d
            ax.Maximum <- maxv + 0.1*d
            

        match getOrders chart with
        | Some(minx,maxx,miny,maxy)  ->
            let ar = chart.ChartAreas.[0]
            sax minx maxx ar.AxisX
            sax miny maxy ar.AxisY                 
        | _ ->()
